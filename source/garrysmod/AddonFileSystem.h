...
public: // IAddonSystem
	...
	bool MountFile( const std::string& gmaPath, std::vector<std::string>* files, uint64_t wsid, uint64_t wsid2, IAddonSystem::AddonSource ) override;
	...
	std::string GetAddonFilepath( uint64_t wsid, bool ) override;
	void UnmountAddon( uint64_t wsid, const char* ) override;
	void UnmountServerAddons( ) override;
	std::string IsAddonValidPreInstall( SteamUGCDetails_t ) override;
